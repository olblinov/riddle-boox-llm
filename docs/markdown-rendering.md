...
Relative local PNG/JPEG/WebP/GIF images render as figures when contained within the Markdown directory. Remote, absolute, hidden, traversal and symlink-escape paths remain labelled placeholders, as do table-cell images and SVG. Images are bounded to 10 MiB and 16M pixels; animated images use their first frame. Raw HTML stays literal text. Mermaid fences render real diagrams through pinned mermaid and puppeteer-core using installed Chrome; BOOX_CHROME_PATH overrides its path. Strict configuration disables diagram directives and blocks page network requests. Invalid diagrams retain a reason and source code. Math, footnotes, task-list checkbox widgets and custom extensions retain visible source syntax. No syntax highlighting, external CSS, or embedded assets. Empty documents show an explicit empty-document message.
