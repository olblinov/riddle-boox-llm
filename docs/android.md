...
Version 0.4.0/code 4 production and test APKs build with the same pinned SDK. Only `--test` is debuggable for isolated QA. Initial SDK log reported pressure 2048, but expanded device logs showed Android 11 denying vendor reflection and an empty raw region. That initial log did not prove working native input. The candidate now requires usable display dimensions, a finite nonempty mapped region, and an invertible coordinate matrix before claiming geometry ready; input stays unverified until a raw callback is observed. The approved compatibility exception now permits native geometry initialization on this device. Optical latency cannot be claimed from startup logs or synthetic Android events.

### Approved app-local compatibility

On 2026-09-06 the user explicitly approved BOOX Review's app-local compatibility exception for a new app version. `BooxApplication` applies `HiddenApiBypass:4.3` only on BOOX Android 11 or later, before the pen SDK starts. Vendor `ResManager.init` and `RxBaseAction.init` also receive the application context, matching the official demo; their inspected initialization code only retains that context. The installed version restricts exemptions to `Landroid/onyx/ViewUpdateHelper;`, `Landroid/view/View;`, `Landroid/view/Surface;`, and `Landroid/view/SurfaceControl;`. Each prefix exempts all hidden members of that named framework class, including the vendor extensions observed in device logs; it is not a method-only exemption. This differs from the official demo's empty-prefix wildcard; connected-device testing confirms geometry initialization with these prefixes. No device-wide policy, root, target-SDK change, or other app is involved. The dependency is pinned by URL and SHA-256 in sdk-lock.json. Compatibility initialization success is not native-input proof; capability and first-callback checks still apply.

Eraser mode suppresses only the SDK's native rendering, retaining raw input. Whole-stroke erasure updates the retained page model. Panel repaint is requested after that Canvas frame is drawn, postponed if a new raw stroke is active; normal pen rendering resumes on eraser-up. This avoids clearing original document pixels or a reader-disable gap. Physical eraser/display verification remains required.
