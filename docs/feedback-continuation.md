...
Automatic wake of an idle desktop task is not enabled. The current stdio MCP connection has no verified mechanism to start a host turn. Local inspection found no default app-server control socket; the desktop's separate IPC socket is not a documented substitute. No alternate daemon, private IPC protocol, or fabricated user message is used.
